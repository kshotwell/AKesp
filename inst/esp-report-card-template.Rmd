...
```{r, eco_ind} 
eco <- list_indicators(data = params$esp_data, indicator_type = "Ecosystem")
cat(eco)
```
...
```{r, socio_ind}
socio <- list_indicators(data = params$esp_data, indicator_type = "Socioeconomic")
cat(socio, sep = "\n\n")
```
...
```{r, table-set-up, message = FALSE, eval = TRUE}
dat <- params$esp_data %>%
...
```{r, overall, fig.height = 5, eval = TRUE, results = "asis"}
alt <- "Alt text"
cap <- c("Simple traffic light score for overall ecosystem and socioeconomic categories from", min(params$esp_data$YEAR), "to present.")

knitr::opts_chunk$set(fig.cap = alt)
try(esp_overall_score(
...
```{r, bayesian, fig.height = 8, eval = TRUE}
render_fig(
