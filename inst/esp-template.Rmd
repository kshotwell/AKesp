...
if (class(try(read.csv(file = fig_spreadsheet)))[1] != "try-error") {
  ...
  for (i in 1:nrow(figs)) {
    if (figs$chunk_name[i] == "conceptual-model") {
       ...
       child <- knitr::knit_child(text = "```{r, con-model}
                                ```",
  quiet = TRUE
)
cat(child, sep = "\n\n")
    } else if (figs$chunk_name[i] == "traffic-eco") {
       child <- knitr::knit_child(text = "```{r, traffic-eco}
                                ```",
  quiet = TRUE
)
cat(child, sep = "\n\n")
    } else if (figs$chunk_name[i] == "traffic-socio") {
       child <- knitr::knit_child(text = "```{r, traffic-socio}
                                ```",
  quiet = TRUE
)
cat(child, sep = "\n\n")
    } else if (figs$chunk_name[i] == "overall") {
       child <- knitr::knit_child(text = "```{r, overall}
                                ```",
  quiet = TRUE
)
cat(child, sep = "\n\n")
    } else if (figs$chunk_name[i] == "bayesian") {
       child <- knitr::knit_child(text = "```{r, bayes}
                                ```",
  quiet = TRUE
)
cat(child, sep = "\n\n")
