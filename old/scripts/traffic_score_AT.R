...
traffic<-read.csv("scripts/sablefish_indicators_2020_new_ma.csv",header=T)
...
teco<-tidyr::pivot_longer(traffic1, cols = -Year)
  #melt(traffic1, id="Year")
tecon<-tidyr::pivot_longer(traffic2, cols = -Year)
  #melt(traffic2, id="Year")
teco$type<-"Ecosystem"
tecon$type<-"Socioeconomic"
t1<-rbind(teco,tecon)

t1 <- dplyr::rename(t1, variable = name)
library(dplyr)
...
npages<-ceiling(length(unique(t2$variable)))
nyears<-max(t2$Year)
